...
delta(k,j) = max(Delta{l-1}(:,k).*v') * Probability5(T1.Token_lemma(l),k-1,j-1,T1.PartOfSpeech(l+1),data_neg,token_dict) ;
...
Total_cv_score_1 = [Total_cv_score_1 cv_score];
Total_jaccard_1 = [Total_jaccard_1 jaccard];
cv_score_end_1 = [cv_score_end_1 cv(size(cv,2))];
